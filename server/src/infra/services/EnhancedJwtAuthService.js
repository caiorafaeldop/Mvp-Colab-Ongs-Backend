...
class EnhancedJwtAuthService {
  ...
  async refreshAccessToken(refreshToken) {
    try {
      ...
      console.log(
        '[REFRESH] JWT_REFRESH_SECRET usado:',
        this.jwtRefreshSecret ? 'DEFINIDO' : 'UNDEFINED'
      );
      ...
      console.log(
        '[REFRESH] Detalhes do usuário:',
        user ? { id: user._id, email: user.email } : 'NENHUM'
      );
  ...
  async generateAccessToken(user) {
    try {
      ...
      console.log(
        '[GENERATE ACCESS TOKEN] JWT_SECRET usado:',
        this.jwtSecret ? 'DEFINIDO' : 'UNDEFINED'
      );
      console.log(
        '[GENERATE ACCESS TOKEN] Tamanho do JWT_SECRET:',
        this.jwtSecret ? this.jwtSecret.length : 0
      );
  ...
  async generateRefreshToken(user) {
    try {
      ...
      console.log(
        '[GENERATE REFRESH TOKEN] JWT_REFRESH_SECRET usado:',
        this.jwtRefreshSecret ? 'DEFINIDO' : 'UNDEFINED'
      );
      ...
      console.log(
        '[GENERATE REFRESH TOKEN] Refresh token gerado com sucesso, tamanho:',
        token.length
      );
      return token;
    } catch (error) {
  ...
  async verifyToken(token) {
    try {
      ...
      console.log(
        '[VERIFY TOKEN] Tamanho do JWT_SECRET:',
        this.jwtSecret ? this.jwtSecret.length : 0
      );
